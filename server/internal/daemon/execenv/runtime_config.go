...
// For Droid:    writes {workDir}/AGENTS.md  (Droid CLI reads AGENTS.md natively; skills discovered from .factory/skills/)
...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if len(ctx.AgentSkills) > 0 {
		...
		case "codex", "copilot", "opencode", "openclaw", "pi", "cursor", "kimi", "kiro", "droid":
			// Codex, Copilot, OpenCode, OpenClaw, Pi, Cursor, Kimi, Kiro, and Droid discover skills natively from their respective paths — just list names.
			b.WriteString("You have the following skills installed (discovered automatically):\n\n")
		case "gemini", "hermes":
